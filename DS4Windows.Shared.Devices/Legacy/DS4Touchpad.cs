...
namespace DS4Windows.Shared.Devices.Legacy
{
    ...
    public class DS4Touchpad
    {
        ...
        public event TouchHandler<TouchpadEventArgs>
            TouchesBegan; // finger one or two landed (or both, or one then two, or two then one; any touches[] count increase)

        public event TouchHandler<TouchpadEventArgs>
            TouchesMoved; // deltaX/deltaY are set because one or both fingers were already down on a prior sensor reading

        public event TouchHandler<TouchpadEventArgs> TouchesEnded; // all fingers lifted
        public event TouchHandler<TouchpadEventArgs> TouchButtonDown; // touchpad pushed down until the button clicks
        public event TouchHandler<TouchpadEventArgs> TouchButtonUp; // touchpad button released

        public event TouchHandler<EventArgs>
            TouchUnchanged; // no status change for the touchpad itself... but other sensors may have changed, or you may just want to do some processing

        public event TouchHandler<EventArgs>
            PreTouchProcess; // used to publish that a touch packet is about to be processed
            ...
            else
            {
                if (touchPadIsDown && !lastTouchPadIsDown)
                {
                    if (TouchButtonDown != null)
                        TouchButtonDown(this,
                            new TouchpadEventArgs(sensors.ReportTimeStamp, sensors.TouchButton, null));
                }
                else if (!touchPadIsDown && lastTouchPadIsDown)
                {
                    if (TouchButtonUp != null)
                        TouchButtonUp(this, new TouchpadEventArgs(sensors.ReportTimeStamp, sensors.TouchButton, null));
                }
